...
export async function startDebate(sectorId: string, title: string, agentIds: string[]): Promise<Debate> {
  try {
    const response = await fetch(`${API_BASE_URL}/debates/start`, {
      ...
      body: JSON.stringify({ sectorId, title, agentIds }),
    });
...
export async function postDebateMessage(debateId: string, agentId: string, content: string, role: string): Promise<Debate> {
  try {
...
export async function closeDebate(debateId: string): Promise<Debate> {
  try {
    ...
    if (!response.ok) {
      let errorMessage = 'Failed to close debate';
      try {
        const errorData = await response.json();
        errorMessage = errorData.error || errorMessage;
      } catch {
...
export async function archiveDebate(debateId: string): Promise<Debate> {
  try {
    const response = await fetch(`${API_BASE_URL}/debates/archive`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      let errorMessage = 'Failed to archive debate';
      try {
        const errorData = await response.json();
        errorMessage = errorData.error || errorMessage;
      } catch {
