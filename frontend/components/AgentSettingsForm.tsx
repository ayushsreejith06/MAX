...
export function AgentSettingsForm({
  ...
  useEffect(() => {
    const loadAgentData = async () => {
      ...
      try {
        // Fetch full agent details to ensure we have all fields
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (target.closest('button[title="Customize agent settings"]')) {
        return;
      }
  ...
  return (
    <div ref={formRef} className="rounded-2xl border border-ink-500 bg-pure-black/60 p-5">
      <div className="flex items-center justify-between mb-4">
        <p className="text-xs uppercase tracking-[0.4em] text-floral-white/50">Agent Settings</p>
        <button
          onClick={onClose}
