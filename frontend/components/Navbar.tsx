...
export default function Navbar() {
  ...
  const navItems = [
    ...
    { href: '/decision-logs', label: 'DECISION LOGS' },
      ...
      <div className="max-w-[1920px] mx-auto px-8 py-4">
        <div className="flex items-center justify-between">
              ...
              {navItems.map((item) => (
                <Link
                  ...
                  className={`text-sm font-semibold uppercase tracking-wide transition-colors font-mono ${
                    ...
                    (item.href === '/decision-logs' && pathname?.startsWith('/decision-logs')) ||
          ...
          <div className="flex items-center gap-6 text-floral-white">
            {/* Balance */}
            <div className="flex items-center gap-2 rounded-full border border-ink-500 px-3 py-1.5 bg-ink-600/40">
              <DollarSign className="w-4 h-4 text-sage-green" />
              <span className="text-sm font-mono text-sage-green">${userBalance.toFixed(2)}</span>
            </div>
